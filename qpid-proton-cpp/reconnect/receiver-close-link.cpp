...
struct receive_handler : public proton::messaging_handler {
    ...
    listener_ready_handler listen_hendler_ {};
    int received_ {0};

    void on_container_start(proton::container &cont) override {
        listener_ = cont.listen(conn_url_ + "/" + address_, listen_hendler_);
    }
    ...
    void on_message(proton::delivery &dlv, proton::message &msg) override {
        ...
        if (expected_ == 0 || received_ < expected_) {
            std::cout << "RCV: " << msg.body();
            received_++;

            // Simulate a link failure with error every err_interval_ messages
            if (received_ % err_interval_ == 0) {
                std::cout << " - closing with error";
                dlv.receiver().close(proton::error_condition("Test close"));
            }
            std::cout << std::endl;
        }
    ...
    void on_receiver_open(proton::receiver &r) override {
        std::cout << "RCV: receiver opened" << std::endl;
        proton::messaging_handler::on_receiver_open(r);
    }
};

int main(int argc, char **argv) {
    std::cout << "receiver-close-link" << std::endl;
    if (argc < 3 || argc > 5) {
